...
from core.models import (
    Riesgo,
    RiesgoListas,
    RiesgoCargasistemacio,
    RiesgoCargasisteman1,
    Gerencias,
    RiesgoEvaluacioncualitativainherente,
    RiesgoEvaluacioncualitativaresidual,
    RiesgoEvaluacioncualitativaobjetivo,
    RiesgoProbImp,
    RiesgoNCausariesgo,
    RiesgoCausas,
    RiesgoUnifica,
    RiesgoColorindicadores,
)
...
def inicio(request):
    ...
    kpi_produccion = list(
        RiesgoKpi.objects.filter(ano=ano, mes=mes, tipo="Producción")
        .order_by('-fecreg')
        .values()
    )
    kpi_seguridad = list(
        RiesgoKpi.objects.filter(ano=ano, mes=mes, tipo="Seguridad")
        .order_by('-fecreg')
        .values()
    )
    kpi_ambiental = list(
        RiesgoKpi.objects.filter(ano=ano, mes=mes, tipo="Ambiental")
        .order_by('-fecreg')
        .values()
    )
    clasificaciones = RiesgoListas.objects.filter(
        tipo="Clasificaciones", estado="ACTIVO"
    ).values("glosa")
    ...
    cantidad_riesgos_acumulados_alto = list(
        RiesgoEvaluacioncualitativaresidual.objects.filter(
            nivelriesgoresidual="Alto"
        ).values_list("idriesgo", flat=True)
    )
    cantidad_riesgos_mensuales_alto = list(
        RiesgoEvaluacioncualitativaresidual.objects.filter(
            modificado__year=datetime.now().year,
            modificado__month=datetime.now().month,
            nivelriesgoresidual="Alto",
        ).values_list("idriesgo", flat=True)
    )
    cantidad_riesgos_acumulados = len(
        list(
            Riesgo.objects.filter(
                idriesgo__in=cantidad_riesgos_acumulados_alto, directic=1
            ).values()
        )
    )
    cantidad_riesgos_mes = len(
        list(
            Riesgo.objects.filter(
                idriesgo__in=cantidad_riesgos_mensuales_alto, directic=1
            ).values()
        )
    )
    cantidad_total_registros = len(
        list(
            Riesgo.objects.filter(
                estadoregistro='Vigente', directic=1
            ).values()
        )
    )
    ...
    dicccionarioMesesSemestre1[2] = {
        'mes': 'Febrero',
        'valor': 0,
    }
    dicccionarioMesesSemestre1[3] = {
        'mes': 'Marzo',
        'valor': 0,
    }
    dicccionarioMesesSemestre1[4] = {
        'mes': 'Abril',
        'valor': 0,
    }
    dicccionarioMesesSemestre1[5] = {
        'mes': 'Mayo',
        'valor': 0,
    }
    dicccionarioMesesSemestre1[6] = {
        'mes': 'Junio',
        'valor': 0,
    }

    dicccionarioMesesSemestre2[7] = {
        'mes': 'Julio',
        'valor': 0,
    }
    dicccionarioMesesSemestre2[8] = {
        'mes': 'Agosto',
        'valor': 0,
    }
    dicccionarioMesesSemestre2[9] = {
        'mes': 'Septiembre',
        'valor': 0,
    }
    dicccionarioMesesSemestre2[10] = {
        'mes': 'Octubre',
        'valor': 0,
    }
    dicccionarioMesesSemestre2[11] = {
        'mes': 'Noviembre',
        'valor': 0,
    }
    dicccionarioMesesSemestre2[12] = {
        'mes': 'Diciembre',
        'valor': 0,
    }
    ...
    for k, v in dicccionarioMesesSemestre1.items():
        valoresMes = RiesgoUnifica.objects.filter(
            fecha__year=ano, fecha__month=k
        ).values()
        for valor in valoresMes:
            totalUnifica += round(float(valor['montoperdidakusd']), 1)
    ...
    if RiesgoKpi.objects.filter(tipo='KUSNETO').exists():
        kusNeto = (
            RiesgoKpi.objects.filter(tipo='KUSNETO')
            .order_by('-fecreg')
            .first()
            .valor
        )
    if RiesgoKpi.objects.filter(tipo='TMFSIN').exists():
        tmfSin = (
            RiesgoKpi.objects.filter(tipo='TMFSIN')
            .order_by('-fecreg')
            .first()
            .valor
        )
...
def kpiAtrasoProyecto(tipo):
    if tipo == "eventos":
        riesgos_atraso_proyecto_acum = list(
            RiesgoUnifica.objects.filter(
                clasificacion="Atraso Proyecto"
            ).values()
        )
        ...
        data = {
            'num_acumulado': num_riesgos_atraso_proyecto_acum,
            'suma_acum': int(suma),
            'num_mes': num_riesgos_atraso_proyecto,
            'suma_mes': int(suma_mes),
            'suma_mes_tmf': int(sumaTmfMes),
            'suma_tmf_total': int(sumaTmfTotal),
        }

    elif tipo == "riesgos":
        riesgos_criticos = list(
            RiesgoEvaluacioncualitativaresidual.objects.filter(
                nivelriesgoresidual="Alto"
            ).values_list("idriesgo", flat=True)
        )
        riesgos_atraso_proyecto_acum = list(
            Riesgo.objects.filter(
                clasificacion="Atraso Proyecto", idriesgo__in=riesgos_criticos
            ).values()
        )
        num_riesgos_atraso_proyecto_acum = len(riesgos_atraso_proyecto_acum)
        suma = 0
        ...
        riesgos_criticos_mensuales = list(
            RiesgoEvaluacioncualitativaresidual.objects.filter(
                modificado__year=datetime.now().year,
                modificado__month=datetime.now().month,
                nivelriesgoresidual="Alto",
        ...
        riesgos_atraso_proyecto = list(
            Riesgo.objects.filter(
                clasificacion="Atraso Proyecto",
                idriesgo__in=riesgos_criticos_mensuales,
            ).values()
        )
        num_riesgos_atraso_proyecto = len(riesgos_atraso_proyecto)
        suma_mes = 0
...
def kpiProbidadTransparencia(tipo):
    if tipo == "eventos":
        riesgos_atraso_proyecto_acum = list(
            RiesgoUnifica.objects.filter(
                clasificacion="Providad y Transparencia"
            ).values()
        )
        num_riesgos_atraso_proyecto_acum = len(riesgos_atraso_proyecto_acum)
        suma = 0
...
def kpiFaltaAgua(tipo):
    if tipo == "eventos":
        riesgos_atraso_proyecto_acum = list(
            RiesgoUnifica.objects.filter(clasificacion="Falta Agua").values()
        )
        ...
        riesgos_atraso_proyecto_acum = list(
            Riesgo.objects.filter(
                clasificacion="Falta Agua", idriesgo__in=riesgos_criticos
            ).values()
        )
        num_riesgos_atraso_proyecto_acum = len(riesgos_atraso_proyecto_acum)
        suma = 0
...
def kpiFallaEquipoCritico(tipo):
    if tipo == "eventos":
        riesgos_atraso_proyecto_acum = list(
            RiesgoUnifica.objects.filter(
                clasificacion="Falla Equipo Crítico"
            ).values()
        )
...
def kpiIncendio(tipo):
    if tipo == "eventos":
        riesgos_atraso_proyecto_acum = list(
            RiesgoUnifica.objects.filter(clasificacion="Incendio").values()
        )
        num_riesgos_atraso_proyecto_acum = len(riesgos_atraso_proyecto_acum)
        suma = 0
        sumaTmfTotal = 0
        ...
        riesgos_atraso_proyecto_acum = list(
            Riesgo.objects.filter(
                clasificacion="Incendio", idriesgo__in=riesgos_criticos
            ).values()
        )
        num_riesgos_atraso_proyecto_acum = len(riesgos_atraso_proyecto_acum)
        suma = 0
...
def kpiUnifica(tipo, clasificacion):
    if tipo == "eventos":
        riesgos_atraso_proyecto_acum = list(
            RiesgoUnifica.objects.filter(clasificacion=clasificacion).values()
        )
        num_riesgos_atraso_proyecto_acum = len(riesgos_atraso_proyecto_acum)
        suma = 0
        sumaTmfTotal = 0
        ...
        riesgos_atraso_proyecto_acum = list(
            Riesgo.objects.filter(
                clasificacion=tipo, idriesgo__in=riesgos_criticos
            ).values()
        )
        num_riesgos_atraso_proyecto_acum = len(riesgos_atraso_proyecto_acum)
        suma = 0
...
def impactoFinancieroEventosMaterializados():
    listas_clasificaciones = list(
        RiesgoListas.objects.filter(tipo="Clasificaciones").values_list(
            "glosa", flat=True
        )
    )
    total = 0
    ...
    for clasificacion in listas_clasificaciones:
        valores = list(
            RiesgoUnifica.objects.filter(
                clasificacion=clasificacion
            ).values_list("montoperdidakusd", flat=True)
        )
    ...
    data = {
        'valores_por_clasificacion': diccionario,
        'total': total,
...
def riesgosGerencias():
    ...
    for gerencia in gerencias:
        total_riesgos_gerencia = len(
            list(Riesgo.objects.filter(gerencia=gerencia['sigla']).values())
        )
        diccionario_riesgos_gerencias[
            gerencia['gerencia']
        ] = total_riesgos_gerencia
...
def getRiesgoClasificaciones():
    ...
    for clasificacion in clasificaciones:
        cantidad_riesgos_acumulado = len(
            list(
                Riesgo.objects.filter(
        ...
        valores_riesgos_acumulado = list(
            Riesgo.objects.filter(
                clasificacion=clasificacion['clasificacion']
            ).values_list("maximaperdidamus", flat=True)
        )
        arreglo_valores_acumulado = [
            float(x or 0) for x in valores_riesgos_acumulado
        ]
        ...
        cantidad_riesgos_mes = len(
            list(
                Riesgo.objects.filter(
                    clasificacion=clasificacion['clasificacion'],
                    fechadigita__month=datetime.now().month,
                    fechadigita__year=datetime.now().year,
                ).values()
            )
        )
        valores_riesgos_mes = list(
            Riesgo.objects.filter(
                clasificacion=clasificacion['clasificacion'],
                fechadigita__month=datetime.now().month,
                fechadigita__year=datetime.now().year,
...
def getRiesgosCriticosPorGerencia(request):
    if request.method == "GET":
        ...
        for gerencia in gerencias:
            ...
            amarillos = 0
            nombre_color = ''
            riesgos_gerencia = list(
                Riesgo.objects.filter(gerencia=gerencia['sigla']).values()
            )

            for riesgo in riesgos_gerencia:
                if RiesgoEvaluacioncualitativaresidual.objects.filter(
                    idriesgo=riesgo['idriesgo']
                ).exists():
                    resultado_inherente_riesgo = list(
                        RiesgoEvaluacioncualitativaresidual.objects.filter(
                            idriesgo=riesgo['idriesgo']
                        ).values('probabilidadresidual', 'impactoresidual')
                    )
                    probabilidad = resultado_inherente_riesgo[0][
                        'probabilidadresidual'
                    ]
                    impacto = resultado_inherente_riesgo[0]['impactoresidual']
                    color = list(
                        RiesgoProbImp.objects.filter(
                            probabilidad=probabilidad, impacto=impacto
                        ).values('color_hex', 'color')
                    )
        ...
        gerencias_ordenadas = sorted(
            dict_criticos.items(), key=operator.itemgetter(1), reverse=True
        )
        cinco_primeras = 0
        ...
        for key in dict_cinco_gerencias.keys():
            porcentaje = round(
                float(
                    (
                        (
                            int(dict_cinco_gerencias[key]['cantidad'])
                            / total_cinco_criticos
                        )
                        * 100
                    )
                ),
            ...
            dict_cinco_gerencias[key] = {
                'cantidad': dict_cinco_gerencias[key]['cantidad'],
                'porcentaje': porcentaje,
                'color': arreglo_colores[i_color],
            }
            i_color += 1
...
def getCausasRiesgosCriticos(request):
    if request.method == "GET":
        ...
        datos_riesgos = RiesgosCriticos(
            tipo_datos, nivel, tipo, ""
        ).getCausasRiesgos()
...
def getMatrices(request):
    if request.method == "GET":
        ...
        data = {
            'matriz_inherente': matriz_inherente,
            'matriz_residual': matriz_residual,
...
def getMatrizInherente(tipo, directic):
    ...
    for gerencia in gerencias:
        id_riesgos = list(
            Riesgo.objects.filter(gerencia=gerencia['sigla']).values_list(
                'idriesgo', flat=True
            )
        )
        if directic == 1:
            id_riesgos = list(
                Riesgo.objects.filter(
                    gerencia=gerencia['sigla'], directic=1
                ).values_list('idriesgo', flat=True)
            )
        ...
        for id_riesgo in id_riesgos:
            ...
            impacto_inherente = ""
            if RiesgoEvaluacioncualitativainherente.objects.filter(
                idriesgo=id_riesgo
            ).exists():
                if (
                    list(
                        RiesgoEvaluacioncualitativainherente.objects.filter(
                            idriesgo=id_riesgo
                        ).values("probabilidad")
                    )[0]["probabilidad"]
                    is not None
            ...
            label_coords = (
                str(impacto_inherente) + '_' + str(probabilidad_inherente)
            )
...
def getMatrizResidual(tipo, directic):
    ...
    for gerencia in gerencias:
        ...
        for id_riesgo in id_riesgos:
            ...
            if RiesgoEvaluacioncualitativaresidual.objects.filter(
                ...
                if (
                    list(
                        RiesgoEvaluacioncualitativaresidual.objects.filter(
                            idriesgo=id_riesgo
                        ).values("probabilidadresidual")
                    )[0]["probabilidadresidual"]
                    is not None
                ):
            ...
            label_coords = (
                str(impacto_residual) + '_' + str(probabilidad_residual)
            )
...
def getMatrizObjetivo(tipo, directic):
    ...
    for gerencia in gerencias:
        ...
        for id_riesgo in id_riesgos:
            ...
            impacto_objetivo = ""
            if RiesgoEvaluacioncualitativaobjetivo.objects.filter(
                idriesgo=id_riesgo
            ).exists():
                if (
                    list(
                        RiesgoEvaluacioncualitativaobjetivo.objects.filter(
                            idriesgo=id_riesgo
                        ).values("probabilidadcontrol")
                    )[0]["probabilidadcontrol"]
                    is not None
                ):
                    probabilidad_objetivo = int(
                        list(
                            RiesgoEvaluacioncualitativaobjetivo.objects.filter(
                                idriesgo=id_riesgo
                            ).values("probabilidadcontrol")
            ...
            if RiesgoEvaluacioncualitativaobjetivo.objects.filter(
                ...
                if (
                    list(
                        RiesgoEvaluacioncualitativaobjetivo.objects.filter(
                            idriesgo=id_riesgo
                        ).values("impactocontrol")
                    )[0]["impactocontrol"]
                    is not None
                ):
                    impacto_objetivo = int(
                        list(
                            RiesgoEvaluacioncualitativaobjetivo.objects.filter(
                                idriesgo=id_riesgo
                            ).values("impactocontrol")
            ...
            label_coords = (
                str(impacto_objetivo) + '_' + str(probabilidad_objetivo)
            )
...
def getDatosDashboardTipoRiesgo(request):
    if request.method == "GET":
        ...
        transformacion_cultural = kpiUnifica(
            "eventos", "Transformacion Cultural"
        )
        ...
        if tipo == "eventos":
            ...
            glosa = "Riesgos"
            cantidad_total_registros = len(
                list(Riesgo.objects.filter(estadoregistro='Vigente').values())
            )
            cantidad_riesgos_acumulados = len(
                list(
                    RiesgoEvaluacioncualitativaresidual.objects.filter(
            ...
            cantidad_riesgos_acumulados = len(
                list(
                    Riesgo.objects.filter(
                        idriesgo__in=cantidad_riesgos_acumulados_alto,
                        directic=1,
                    ).values()
                )
            )
            cantidad_riesgos_mes = len(
                list(
                    Riesgo.objects.filter(
                        idriesgo__in=cantidad_riesgos_mensuales_alto,
                        directic=1,
                    ).values()
                )
            )
        ...
        data = {
            ...
            'cantidad_total_registros': cantidad_total_registros,
            "glosa": glosa,
            "falta_mineral": falta_mineral,
            "transformacion_cultural": transformacion_cultural,
        }
...
def detalleCoordenadaMatriz(request):
    if request.method == "GET":
        ...
        if tipo_matriz == "res":
            id_riesgo = list(
                RiesgoEvaluacioncualitativaresidual.objects.filter(
                    probabilidadresidual=X, impactoresidual=Y
                ).values_list("idriesgo", flat=True)
            )
        if tipo_matriz == "inhe":
            id_riesgo = list(
        ...
        if tipo_matriz == "obj":
            id_riesgo = list(
                RiesgoEvaluacioncualitativaobjetivo.objects.filter(
                    probabilidadcontrol=X, impactocontrol=Y
                ).values_list("idriesgo", flat=True)
            )

        if gerencia == "DET":
            lista_riesgos = list(
                Riesgo.objects.filter(idriesgo__in=id_riesgo).values()
            )
        else:
            lista_riesgos = list(
                Riesgo.objects.filter(
                    idriesgo__in=id_riesgo, gerencia=gerencia
                ).values()
            )
        ...
        if directic == 1:
            if gerencia == "DET":
                lista_riesgos = list(
                    Riesgo.objects.filter(
                        idriesgo__in=id_riesgo, directic=1
                    ).values()
                )
            else:
                lista_riesgos = list(
                    Riesgo.objects.filter(
                        idriesgo__in=id_riesgo, gerencia=gerencia, directic=1
                    ).values()
                )
...
def getDatosCampo(request):
    if request.method == "GET":
        ...
        datos_campo = list(
            RiesgoUnifica.objects.values_list(campo, flat=True).distinct()
        )
        ...
        for campo_consulta in datos_campo:
            query = (
                'list(RiesgoUnifica.objects.filter('
                + campo
                + '=\''
                + str(campo_consulta)
                + '\').values("evento", "montoperdidakusd"))'
            )
            datos = eval(query)
            if valor == "DET":
                query = (
                ...
                if tipo == "GERENCIA":
                    query = (
                        'list(RiesgoUnifica.objects.filter(gerencia=\''
                        + str(valor)
                        + '\','
                        + campo
                        + '=\''
                    ...
                    query = (
                        'list(RiesgoUnifica.objects.filter(superintendencia=\''
                        + str(valor)
                        + '\','
                        + campo
                        + '=\''
            ...
            for dato in datos:
                if isinstance(campo_consulta, date):
                    campo_consulta = (
                        str(campo_consulta.day)
                        + '/'
                        + str(campo_consulta.month)
                        + '/'
                        + str(campo_consulta.year)
                    )
                if contador < 10:
                    ...
                    if campo_consulta in diccionario_datos_montos:
                        if dato['montoperdidakusd'] != "-":
                            diccionario_datos_montos[campo_consulta] += float(
                                dato['montoperdidakusd']
                            )
                    else:
                        if dato['montoperdidakusd'] != "-":
                            diccionario_datos_montos[campo_consulta] = float(
        ...
        for k, v in diccionario_datos_montos.items():
            diccionario_datos_montos[k] = int(v)

        data = {
            'cantidad': diccionario_datos_contador,
            'montokus': diccionario_datos_montos,
    ...
    if request.method == "GET":
        ...
        if valor == "DET":
            query = (
                ...
                + '=\''
                + str(glosa)
                + '\').order_by("evento").values("evento", "montoperdidakusd"))'
            )
            datos = eval(query)
        else:
            if tipo == "GERENCIA":
        ...
        for dato in datos:
            ...
            if dato['evento'] in diccionario_datos_montos:
                diccionario_datos_montos[dato['evento']] += float(
                    dato['montoperdidakusd']
                )
            else:
                diccionario_datos_montos[dato['evento']] = float(
        ...
        contadorIndices = 0
        arregloIndices = []
        for k, v in diccionario_datos_contador.items():
            if contadorIndices < 10:
                contadorIndices += 1
                arregloIndices.append(k)
        diccionarioContador = OrderedDict()
        diccionarioMontos = OrderedDict()
        for indice in arregloIndices:
            diccionarioContador[indice] = diccionario_datos_contador[indice]
            diccionarioMontos[indice] = int(diccionario_datos_montos[indice])

        data = {'cantidad': diccionarioContador, 'montokus': diccionarioMontos}
        return JsonResponse(data, safe=False)
    ...
    if request.method == "GET":
        ...
        for mes in meses:
            clasificaciones = list(
                RiesgoUnifica.objects.filter(fecha__month=mes)
            ...
            for clasificacion in clasificaciones:
                ...
                for datosClas in datosClasificacion:
                    if (
                        datosClas['evento']
                        in dictDatos[clasificacion['clasificacion']]
                    ):
                        dictDatos[clasificacion['clasificacion']][
                            datosClas['evento']
                        ]['valor'] += round(
                            float(datosClas['montoperdidakusd']), 1
                        )
                        dictDatos[clasificacion['clasificacion']][
                            datosClas['evento']
                        ]['q'] += 1
...
def listadoClasificacionGerencia(request):
    if request.method == "GET":
        ...
        if todos == "false":
            ...
            meses = []
            fechas = list(
                RiesgoUnifica.objects.filter(gerencia=gerencia)
                .values("fecha")
                .distinct()
            )
            for fecha in fechas:
        ...
        for mes in meses:
            dictDatosGerencia = {}
            datosClasificacion = []
            if RiesgoUnifica.objects.filter(
                fecha__month=mes, fecha__year=ano, gerencia=gerencia
            ).exists():
                datosClasificacion = list(
                    RiesgoUnifica.objects.filter(
                        fecha__month=mes, gerencia=gerencia
                    ).values()
                )
            for dato in datosClasificacion:
                if dato['clasificacion'] in dictDatosGerencia:
                    dictDatosGerencia[dato['clasificacion']]['valor'] += round(
                        float(dato['montoperdidakusd']), 1
                    )
                    dictDatosGerencia[dato['clasificacion']]['q'] += 1
                    dictDatosGerencia[dato['clasificacion']]['mes'] = str(
                        dato['fecha'].month
                    )
                else:
