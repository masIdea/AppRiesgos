...
def saveRiesgo(request):
    if request.method == "POST":
        ...
        if riesgo.pk is not None:
            Auditoria(str(request.user), "Crea Riesgo "+str(riesgo_descripcion), "CreaRiesgo", riesgo.pk).saveAudit()            
...
def registraCausaConsecuencia(request):
    ...
    if tipo_valor == "CAUSA":        
        ...
        Auditoria(str(request.user), "Registra Causa Al Riesgo "+str(identificador_riesgo), "CreaCausa", valor_registro).saveAudit()
    ...
    if  tipo_valor == "CONSECUENCIA":        
        ...
        Auditoria(str(request.user), "Registra Consecuencia Al Riesgo "+str(identificador_riesgo), "CreaConsecuencia", valor_registro).saveAudit()
...
def eliminaCausaConsecuencia(request):
    ...
    try:        
        if tipo_valor == "CAUSA":
            ...
            Auditoria(str(request.user), "Elimina Causa del Riesgo "+str(identificador_riesgo), "EliminaCausaRiesgo", valor[1]).saveAudit()
        if  tipo_valor == "CONSECUENCIA":        
            RiesgoNRiesgoconsecuencia.objects.filter(idconsecuencia = valor[1], idriesgo = identificador_riesgo).delete()
            Auditoria(str(request.user), "Elimina Consecuencia del Riesgo "+str(identificador_riesgo), "EliminaConsecuenciaRiesgo", valor[1]).saveAudit()
        valida = True
...
def saveRiesgoInherente(request):
    if request.method == "POST":                        
        ...
        if save_riesgo_inherente.pk is not None:
            Auditoria(str(request.user), "Registra Matriz Inherente del Riesgo "+str(id_riesgo), "MatrizInherente", save_riesgo_inherente.pk).saveAudit()            
